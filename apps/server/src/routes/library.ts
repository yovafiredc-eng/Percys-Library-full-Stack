...
libraryRouter.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    // Background cover pre-generation for newly added comics
    // This runs asynchronously so it doesn't block the response
    const newComicIds: string[] = [];
    for (let i = 0; i < results.length; i++) {
      const result = results[i];
      if (result.status === "fulfilled" && (result.value === "added" || result.value === "updated")) {
        const source = sources[i];
        const comic = await prisma.comic.findFirst({ where: { ownerId, path: source.sourcePath }, select: { id: true } });
        if (comic) newComicIds.push(comic.id);
      }
    }
    // Generate covers in background with small delays to not overload the server
    if (newComicIds.length > 0) {
      (async () => {
        for (const comicId of newComicIds) {
          try {
            await getCover(comicId);
          } catch { /* ignore background errors */ }
          // Small delay between covers to spread CPU load
