...
export async function registerComicPath(
  ...
  const clampedPage = hasPages
    ? Math.min(Math.max(0, existing.currentPage), pageCount - 1)
    : 0;
  // A book with zero detected pages is NOT "completed"; treating it as
  // such was an off-by-one bug — `pageCount - 1 === -1` and `0 >= -1`
  // evaluates to true, which silently flipped every unreadable / still-
  // extracting comic to the completed state on every scan.
  const normalizedCompleted = hasPages
    ? clampedPage >= pageCount - 1 && existing.completed
    : false;
