...
/** Maximum time to wait for an image before treating it as failed.
 * Prevents the reader from being stuck on a loading spinner forever
 * when a page image hangs or the server is unresponsive.
 * 20s allows large pages / slow connections to resolve without
 * premature error states. */
...
export function ReaderPageImage({
  ...
  useEffect(() => {
    ...
    // Hard timeout: if the image hasn't loaded within IMAGE_TIMEOUT_MS,
    // force an error state so the reader doesn't stay stuck forever.
    const timeoutTimer = window.setTimeout(() => {
      handlePendingError(pendingTokenRef.current);
    }, IMAGE_TIMEOUT_MS);
    return () => {
